...
import br.com.lelo.primefaces.business.CompradorBusiness;
...
public class LoginManagedBean {
	...
	public String login() {
		...
		if (compradorBusiness.isBlank(getComprador()) == false) {
			...
			if (byLogin == null) {
				FacesContextMessageUtil.addInfoMessage("login:txtLogin", "Usu�rio n�o cadastrado");
			} else {

				if (getComprador().getPassword().equalsIgnoreCase(byLogin.getPassword()) == false) {
					FacesContextMessageUtil.addInfoMessage("login:txtUserPassword", "Senha inv�lida");
				} else {
